...
- **G-1**: 各問について 5%(`TRICK_RATE = 0.05`)の確率で、2択のうちどちらか1枚が「化けている」状態になる。正体はゾロアークまたはヒスイゾロアークからランダムに選ぶ。
...
- **G-5**: 整合性維持のため、正体の実速度が `target` より速い場合は速い側(fast 役)、遅い場合は遅い側(slow 役)の選択肢にのみ化ける。これにより出現後も `fast.speed > target.speed > slow.speed` が成立する。化けの皮は同じ役回りの元ポケモンなので、出現前の見た目上も矛盾しない。
- **G-6**: 正体と3匹の間で ID 重複または同速(C-1/C-2 違反)が生じる場合、その問ではギミックを発生させない。
...
- `state.ts`: `answer()` は未出現の disguise があれば正誤判定せず `"revealed"` を返す
