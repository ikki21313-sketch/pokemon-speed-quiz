...
**振り方バイアス対策（ラベル先決め・独立割り当て）**: 4枚の振り方と正解の位置は、ポケモンを探す**前に**互いに独立なランダムで確定する。これにより振り方ラベルと正誤が無相関になり、「最速を選べばだいたい正解」のようなメタ読みが成立しない（実測: P(正解|各ラベル) ≒ 25%、テスト T-8 で常時検証）。C-7 のレンジ制限は「どの振り方×速い/遅いの窓にも該当ポケモンが存在する」ことを保証してこの独立性を成立させるための条件で、副作用として極端に遅い/速いお手本（無振りツボツボ等）は出題されない。
