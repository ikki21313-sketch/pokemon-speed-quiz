...
- **G-2**: 化けている選択肢は、見た目(立ち絵・名前・振り方バッジ)は化けの皮のまま表示するが、実体(実数値・正誤判定)は正体のものを使う。**化けの皮には ±20(C-4)に収まらない明らかに遅いポケモンをあえて表示する**(鋭いプレイヤーへの違和感ヒント)。
- **G-3**: 化け発生時、**見かけの4体は全てお手本より遅い**(=見かけ上正解がない)。正体の2体は「**片方だけがお手本より速い**」振り方を取り(C-5 維持)、出現後は 105族 vs 110族 × 振り方の計算勝負になる。どちらのフォルムが正解になるかはランダム。
- **G-4**: 化けている問題で回答すると、正誤を問わずその回答は確定させず、化けている2枚に同時にノイズ演出が走って正体が出現する。回答せずに **10秒放置**(`REVEAL_IDLE_MS`)しても自動で出現する(見かけの正解がなく詰まるため)。
- **G-5**: 正体出現後、その問題は再び回答可能になる。2回目の回答で正誤を確定する。**正体の振り方は C-8(最速1・準速2・無振り1)に縛られず**、実数値も C-4(±20)の対象外。見かけの構成(化けの皮込み)は C-8 を維持する。
- **G-6**: 両フォルムの計算値は 125〜178 のため、「片方が速く片方が遅い」を満たせるのは**お手本の計算値が約126〜175 の問題のみ**。条件を満たせない場合(ID 重複・同計算値含む)、その問ではギミックを発生させない。
- **G-7**: 結果画面の履歴では正体を表示し、化けていた2体に「ばけていた！」タグを付ける。
...
- `question.ts`: `buildQuestions(..., { tricks, trickRate })` でギミック注入(純粋関数のまま)。正体2体の速い/遅い割り当てを先に決め、見かけの4体は全て遅く生成する(化けの皮2枚は範囲外の遅さ)
...
- `quiz.ts`: 出現前は化けの皮(振り方バッジ含む)を表示、出現時に2枚同時の `.glitch` アニメーション+「どっちが 〇〇より はやい？」バナー
- テスト Z-1〜Z-5 でセット出現・正解配置(片方のみ速い)・両フォルムが正解になり得ること・不変条件を検証
