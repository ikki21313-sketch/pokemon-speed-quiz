...
- お手本1体+選択肢4体を選出する。ただし以下をすべて満たすこと。
  ...
  - **C-5**: お手本より計算値が速い選択肢は、正体出現後の実体で**最大1体**。`TARGET_WIN_RATE`（=0.2、5枠一様相当）の確率で**「お手本自身が最速」の問題**になり、その場合は0体で正解はお手本自身をタップすること。**0 にすると旧仕様（常に選択肢の1枚が正解）へ切り戻せる**（コミットタグ `pre-target-answer` でも復元可）。この仕様は「4枚中の最速を選べばお手本を見ずに解ける」抜け道を塞ぎ、お手本の計算を必須にするためのもの
...
| お手本カード | 振り方バッジ・立ち絵・名前・図鑑No、実数値は `???` 表示。**タップで「お手本が最速」と回答できる** | 実数値+種族値と速度バーを表示。お手本が正解なら緑枠、お手本を選んでいたらバッジ表示 |
| 設問文 | 「〇〇より すばやいのは どれ？（いなければ お手本を タップ！）」 | 同左 |
...
- **G-1**: 各問について 5%(`TRICK_RATE = 0.05`)の確率で化けギミックが発生する。**ゾロアークとヒスイゾロアークは必ずセット**で、選択肢4枚のうち2枚に化ける。ゾロアークはこの経路でのみ登場する(C-6)。
- **G-2**: 化けている選択肢は、見た目(立ち絵・名前・振り方バッジ)は化けの皮のまま表示するが、実体(実数値・正誤判定)は正体のものを使う。**化けの皮には ±20(C-4)に収まらない「明らかに速い」または「明らかに遅い」ポケモンをあえて表示する**(50%ずつ。速い皮が枠の振り方で見つからない場合は遅い皮)。明らかに速い皮は「これが正解だろう」と選ばせる釣りとして機能する。
- **G-3**: 正解位置は通常問題と同じく一様ランダム(お手本最速の問題も含む)。正解がゾロアーク枠ならその正体だけが速く(見かけの実体に正解なし)、正解が通常枠・お手本ならゾロアーク2体は両方遅い(C-5 維持)。**ゾロアーク以外(お手本含む)も正解になり得る**ため、出現しても正解の在り処は分からず全4枚+お手本の再計算が必要になる。
- **G-4**: 未出現の化け問題での1発目の回答は次のように分岐する。
  - **化けていないカードで正解を当てた場合**: 出現をスキップし、**仮の問題のまま正解として確定**する(素早さ表示・結果画面まで化けの皮の数値で通し、ゾロアークの存在は最後まで明かさない)。
  - **それ以外**(不正解、または化けているカードを選んだ — 偶然それが隠れた正解でも): 回答は確定させず、化けている2枚に同時にノイズ演出が走って正体が出現する。
  - **見た目上速そうなカードが1枚もない**場合のみ、回答せず **10秒放置**(`REVEAL_IDLE_MS`)でも自動で出現する(詰み防止。速い皮が見えている場合はクリックに委ねる)。
- **G-5**: 正体出現後、その問題は再び回答可能になる(バナー「ここからが ほんとうの しょうぶ！」)。2回目の回答で正誤を確定する。**正体の振り方は C-8(最速1・準速2・無振り1)に縛られず**、実数値も C-4(±20)の対象外。見かけの構成(化けの皮込み)は C-8 を維持する。
- **G-6**: 正解位置の一様性を保つため、**両フォルムが「速くも遅くもなれる」お手本計算値(約131〜171)の問題でのみ発生**する。条件を満たせない場合(ID 重複・同計算値含む)、その問ではギミックを発生させない。
- **G-7**: 結果画面の履歴では、**出現した問題**は正体の数値を表示して「ばけていた！」タグを付け、「ばける まえ（仮の問題）」として化けの皮2体の数値を併記する。**出現せずスキップした問題**は仮の問題のまま表示し、種明かしをしない(なかったものとして扱う)。
...
- `types.ts`: `Disguise { slots: {pos, shown: Entry}[], revealed }` を `Question.disguise` に追加(slots は常に2枠)
- `question.ts`: `buildQuestions(..., { tricks, trickRate })` でギミック注入(純粋関数のまま)。正解位置を先に決め、ゾロアーク2体は「正解枠なら速く・それ以外は遅い」振り方を取る
- `state.ts`: `answer()` は G-4 の分岐を実装。`hasVisibleFaster()` が放置タイマーの発動条件、`revealDisguise()` はタイマーからも呼ばれる
- `main.ts`: 条件を満たす化け問題の表示時に 10 秒の放置タイマーを張り、発火で自動リビール(再描画時にクリア)
- `quiz.ts`: 出現前は化けの皮(振り方バッジ含む)を表示、出現時に2枚同時の `.glitch` アニメーション+「ここからが ほんとうの しょうぶ！」バナー。未出現のまま確定した問題は化けの皮の数値を表示する
- テスト Z-1〜Z-6(セット出現・正解配置・両フォルム正解・皮の両側性・不変条件)、S-1〜S-6(スキップ/出現/再回答のフロー)で検証
